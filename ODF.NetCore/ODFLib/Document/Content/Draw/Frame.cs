...
namespace ODFLib.Document.Content.Draw
{
	/// <summary>
	...
	public class Frame : IContent, IContentContainer
	{
		...
		public string LoadImageFromFile(string graphicfilename)
		{
			try
			{
				...
				this._width		= ODFLib.Document.Helper.SizeConverter.GetWidthInCm(image.Width, this._dPI_X);
				this._height	= ODFLib.Document.Helper.SizeConverter.GetHeightInCm(image.Height, this._dPI_Y);
				...
				else
				{
					...
					if(ODFLib.Document.Helper.SizeConverter.IsCm(this.SvgWidth))
					{
						this._measurementFormat = "cm";
					}
					else
