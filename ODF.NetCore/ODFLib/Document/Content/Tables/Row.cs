...
namespace ODFLib.Document.Content.Tables
{
	/// <summary>
	...
	public class Row : IContent, IHtml
	{
		...
		private void InitStandards()
		{
			...
			this.CellCollection.Removed		+=new ODFLib.Document.Collections.CollectionWithEvents.CollectionChange(CellCollection_Removed);
			this.CellCollection.Inserted	+=new ODFLib.Document.Collections.CollectionWithEvents.CollectionChange(CellCollection_Inserted);

//is possible!!!! 
//			if(this.Document is AODL.Document.TextDocuments.TextDocument)
//			{
				this.CellSpanCollection				= new CellSpanCollection();
				this.CellSpanCollection.Inserted	+=new ODFLib.Document.Collections.CollectionWithEvents.CollectionChange(CellSpanCollection_Inserted);
				this.CellSpanCollection.Removed		+=new ODFLib.Document.Collections.CollectionWithEvents.CollectionChange(CellSpanCollection_Removed);
//			}
		}
		...
		public void MergeCells(ODFLib.Document.TextDocuments.TextDocument document,int cellStartIndex, int mergeCells, bool mergeContent)
		{
			try
			{
				...
				for(int i=cellStartIndex+mergeCells-1; i>cellStartIndex; i--)
				{
					...
					this.CellSpanCollection.Add(new CellSpan(this, (ODFLib.Document.TextDocuments.TextDocument)this.Document));
				}
			}
			catch(Exception ex)
			{
