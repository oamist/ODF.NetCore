...
namespace ODFLib.Document.Content.Text
{
	/// <summary>
	...
	public class Paragraph : IContent, IContentContainer, IHtml, ITextContainer, ICloneable
	{
		...
		private void InitStandards()
		{
			...
			if(this.Document is ODFLib.Document.TextDocuments.TextDocument)
				this.Document.DocumentMetadata.ParagraphCount	+= 1;

			this.TextContent.Inserted	+=new ODFLib.Document.Collections.CollectionWithEvents.CollectionChange(TextContent_Inserted);
			this.Content.Inserted		+=new ODFLib.Document.Collections.CollectionWithEvents.CollectionChange(Content_Inserted);
			this.TextContent.Removed	+=new ODFLib.Document.Collections.CollectionWithEvents.CollectionChange(TextContent_Removed);
...
//			}
		...
		private string GetHtmlStyleFromGlobalStyles()
		{
			try
			{
				...
				if(this.Document is ODFLib.Document.TextDocuments.TextDocument)
				{
					XmlNode styleNode	= ((ODFLib.Document.TextDocuments.TextDocument)this.Document).DocumentStyles.Styles.SelectSingleNode(
						"//office:styles/style:style[@style:name='"+this.StyleName+"']", this.Document.NamespaceManager);

					if(styleNode == null)
						styleNode	= ((ODFLib.Document.TextDocuments.TextDocument)this.Document).DocumentStyles.Styles.SelectSingleNode(
					...
					if(styleNode != null)
					{
						...
						if(parentNode != null)
							if(parentNode.InnerText != null)
							{
								...
								parentStyleNode	= ((ODFLib.Document.TextDocuments.TextDocument)this.Document).DocumentStyles.Styles.SelectSingleNode(
