...
namespace ODFLib.Document.Export.OpenDocument
{
	/// <summary>
	...
	public class OpenDocumentTextExporter : IExporter, IPublisherInfo
	{
		...
		public void Export(ODFLib.Document.IDocument document, string filename)
		{
			try
			{
				...
				else if(document is ODFLib.Document.SpreadsheetDocuments.SpreadsheetDocument)
				{
					this.WriteSingleFiles(((ODFLib.Document.SpreadsheetDocuments.SpreadsheetDocument)document).DocumentManifest.Manifest, dir+DocumentManifest.FolderName+"\\"+DocumentManifest.FileName);
					this.WriteSingleFiles(((ODFLib.Document.SpreadsheetDocuments.SpreadsheetDocument)document).DocumentMetadata.Meta, dir+DocumentMetadata.FileName);
					this.WriteSingleFiles(((ODFLib.Document.SpreadsheetDocuments.SpreadsheetDocument)document).DocumentSetting.Settings, dir+DocumentSetting.FileName);
					this.WriteSingleFiles(((ODFLib.Document.SpreadsheetDocuments.SpreadsheetDocument)document).DocumentStyles.Styles, dir+DocumentStyles.FileName);
					this.WriteSingleFiles(((ODFLib.Document.SpreadsheetDocuments.SpreadsheetDocument)document).XmlDoc, dir+"content.xml");
				}
				else
...
//				this.SaveExistingGraphics(document.DocumentThumbnails, dir+"Thumbnails\\");
		...
		private void WriteMimetypeFile(string file)
		{
			...
			try
			{
				...
				if(this._document is ODFLib.Document.TextDocuments.TextDocument)
				{
					sw.WriteLine("application/vnd.oasis.opendocument.text");
				}
				else if(this._document is ODFLib.Document.SpreadsheetDocuments.SpreadsheetDocument)
				{
