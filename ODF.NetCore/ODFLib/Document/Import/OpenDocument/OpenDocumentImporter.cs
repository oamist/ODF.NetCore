...
using ODFLib.Document.Import;
using ODFLib.Document.Export;
using ODFLib.Document.Exceptions;
using ODFLib.Document.Import.OpenDocument.NodeProcessors;
...
namespace ODFLib.Document.Import.OpenDocument
{
	/// <summary>
...
//			}
		...
		private void ReadResources()
		{
			try
			{
				...
				{
					((TextDocument)this._document).DocumentSetting				= new  ODFLib.Document.TextDocuments.DocumentSetting();
					string file		= ODFLib.Document.TextDocuments.DocumentSetting.FileName;
					((TextDocument)this._document).DocumentSetting.LoadFromFile(dir+file);

					((TextDocument)this._document).DocumentManifest				= new ODFLib.Document.TextDocuments.DocumentManifest();
					string folder	= ODFLib.Document.TextDocuments.DocumentManifest.FolderName;
					file			= ODFLib.Document.TextDocuments.DocumentManifest.FileName;
					((TextDocument)this._document).DocumentManifest.LoadFromFile(dir+folder+"\\"+file);

					((TextDocument)this._document).DocumentStyles				= new ODFLib.Document.TextDocuments.DocumentStyles();
					file			= ODFLib.Document.TextDocuments.DocumentStyles.FileName;
					((TextDocument)this._document).DocumentStyles.LoadFromFile(dir+file);
				}
				else if(this._document is SpreadsheetDocument)
				{
					((SpreadsheetDocument)this._document).DocumentSetting				= new  ODFLib.Document.SpreadsheetDocuments.DocumentSetting();
					string file		= ODFLib.Document.SpreadsheetDocuments.DocumentSetting.FileName;
					((SpreadsheetDocument)this._document).DocumentSetting.LoadFromFile(dir+file);

					((SpreadsheetDocument)this._document).DocumentManifest				= new ODFLib.Document.SpreadsheetDocuments.DocumentManifest();
					string folder	= ODFLib.Document.SpreadsheetDocuments.DocumentManifest.FolderName;
					file			= ODFLib.Document.SpreadsheetDocuments.DocumentManifest.FileName;
					((SpreadsheetDocument)this._document).DocumentManifest.LoadFromFile(dir+folder+"\\"+file);

					((SpreadsheetDocument)this._document).DocumentStyles				= new ODFLib.Document.SpreadsheetDocuments.DocumentStyles();
					file			= ODFLib.Document.SpreadsheetDocuments.DocumentStyles.FileName;
		...
		private void ReadContent()
		{
			try
			{
                ...
                using ( Stream str = ass.GetManifestResourceStream( "ODFLib.Resources.OD.tweakcontent.xsl" ) )
				...
				mcp.OnWarning					+=new ODFLib.Document.Import.OpenDocument.NodeProcessors.MainContentProcessor.Warning(mcp_OnWarning);
				TextContentProcessor.OnWarning	+=new ODFLib.Document.Import.OpenDocument.NodeProcessors.TextContentProcessor.Warning(TextContentProcessor_OnWarning);
		...
		private void mcp_OnWarning(ODFLib.Document.Exceptions.AODLWarning warning)
		{
			this._importError.Add(warning);
		}

		private void TextContentProcessor_OnWarning(ODFLib.Document.Exceptions.AODLWarning warning)
		{
			this._importError.Add(warning);
		}		
	}
