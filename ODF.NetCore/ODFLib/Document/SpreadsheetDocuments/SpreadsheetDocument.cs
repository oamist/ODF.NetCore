...
using ODFLib.Document.Import.OpenDocument;
...
namespace ODFLib.Document.SpreadsheetDocuments
{
	/// <summary>
	...
	public class SpreadsheetDocument : IDocument, IDisposable
	{
		...
		private ODFLib.Document.SpreadsheetDocuments.DocumentStyles _documentStyles;
		/// <summary>
		/// Gets or sets the document styes.
		/// </summary>
		/// <value>The document styes.</value>
		public ODFLib.Document.SpreadsheetDocuments.DocumentStyles DocumentStyles
		{
			get { return this._documentStyles; }
			set { this._documentStyles = value; }
		}

		private ODFLib.Document.SpreadsheetDocuments.DocumentSetting _documentSetting;
		/// <summary>
		/// Gets or sets the document setting.
		/// </summary>
		/// <value>The document setting.</value>
		public ODFLib.Document.SpreadsheetDocuments.DocumentSetting DocumentSetting
		{
		...
		private ODFLib.Document.SpreadsheetDocuments.DocumentManifest _documentManifest;
		/// <summary>
		/// Gets or sets the document manifest.
		/// </summary>
		/// <value>The document manifest.</value>
		public ODFLib.Document.SpreadsheetDocuments.DocumentManifest DocumentManifest
		{
		...
		public SpreadsheetDocument()
		{
			...
			this.TableCollection.Inserted	+=new ODFLib.Document.Collections.CollectionWithEvents.CollectionChange(TableCollection_Inserted);
			this.TableCollection.Removed	+=new ODFLib.Document.Collections.CollectionWithEvents.CollectionChange(TableCollection_Removed);
			this.Content.Inserted			+=new ODFLib.Document.Collections.CollectionWithEvents.CollectionChange(Content_Inserted);
			this.Content.Removed			+=new ODFLib.Document.Collections.CollectionWithEvents.CollectionChange(Content_Removed);
		}
		...
		private void LoadBlankContent()
		{
			try
			{
				...
				Stream str				= ass.GetManifestResourceStream("ODFLib.Resources.OD.spreadsheetcontent.xml");
		...
		private void Dispose(bool disposing)
		{
			if(!this._disposed)
			{
				if(disposing)
				{
					try
					{
						ODFLib.Document.Export.OpenDocument.OpenDocumentTextExporter.CleanUpReadAndWriteDirectories();
					}
					catch(Exception ex)
					{
						throw ex;
		...
		/// <summary>
		/// Releases unmanaged resources and performs other cleanup operations before the
		/// <see cref="ODFLib.Document.TextDocuments.TextDocument"/> is reclaimed by garbage collection.
		/// </summary>
		~SpreadsheetDocument()      
		{
			Dispose();
